...
for passIx in xrange(3):
    for ix in xrange(100 if passIx == 0 else 50 if passIx == 1 else 10):
        failureRate = .9
        if '0' in str(ix):
            failureRate = 1.0
        if '3' in str(ix):
        ...
        for logfile_ix in xrange(1 if '2' not in str(ix) else 3):
    	    ...
    	    with open(path,"w") as f:
    	    	print >> f, "log results for %stest_%d slice %s, passIx %s" % (group, ix, logfile_ix, passIx)
    	    paths.append(path)
