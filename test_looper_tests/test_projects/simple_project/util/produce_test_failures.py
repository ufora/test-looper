...
for passIx in range(3):
    for ix in range(100 if passIx == 0 else 50 if passIx == 1 else 10):
        ...
        for logfile_ix in range(1 if "2" not in str(ix) else 3):
            path = os.path.join(
                os.getenv("TEST_OUTPUT_DIR"),
                "logfile_%s_%s_%s.stdout" % (ix, logfile_ix, passIx),
            )
            with open(path, "w") as f:
                print(
                    "log results for %stest_%d slice %s, passIx %s"
                    % (group, ix, logfile_ix, passIx),
                    file=f,
                )
            paths.append(path)
        ...
        testDefs.append(
            {
                "testName": "%stest_%d" % (group, ix),
                "success": random.random() < failureRate,
                "logs": paths,
            }
        )
