...
test_dir = os.path.join(
    os.path.split(os.path.split(test_looper.__file__)[0])[0], "test_looper_tests"
)
...
class DockerTests(unittest.TestCase):
    ...
    def test_subdocker(self):
        image = Docker.DockerImage.from_dockerfile_as_string(
            None, dockerfile, create_missing=True
        )
    ...
    def test_docker_killall(self):
        ...
        with DockerWatcher.DockerWatcher(name_prefix) as watcher:
            ...
            self.assertTrue(
                Docker.killAllWithNamePrefix(name_prefix) == 1, container.logs()
            )
        ...
        with DockerWatcher.DockerWatcher("namespace_1") as watcher1:
            with DockerWatcher.DockerWatcher("namespace_2") as watcher2:
                listener1 = watcher1.run(
                    ...
                    [
                        "python",
                        "/test_dir/test_projects/simple_project/socket_listener.py",
                        "8000",
                        "listener1",
                    ],
                ...
                sender1 = watcher1.run(
                    ...
                    [
                        "python",
                        "/test_dir/test_projects/simple_project/socket_sender.py",
                        "listener",
                        "8000",
                        "listener1",
                    ],
                ...
                self.assertEqual(
                    ...
                    + "\n".join(
                        [
                            "*******\n" + x
                            for x in [
                                l.logs(stdout=True, stderr=True).decode("ASCII")
                                for l in [listener1, listener2, sender1, sender2]
                            ]
                        ]
                    ),
                )
        ...
        with DockerWatcher.DockerWatcher("namespace_1") as watcher1:
            with DockerWatcher.DockerWatcher("namespace_2") as watcher2:
                listener1 = watcher1.run(
                    ...
                    [
                        "python",
                        "/test_dir/test_projects/simple_project/socket_listener_in_new_container.py",
                        image.image,
                        "child",
                        "8000",
                        "listener1",
