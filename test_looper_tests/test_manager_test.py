...
class TestManagerTests(unittest.TestCase):
    def test_manager_refresh(self):
        ...
        self.assertEqual(
            sorted(phases[2]),
            sorted(["repo2/c1/test/linux", "repo2/c0/test/linux"]),
            phases,
        )
    ...
    def test_manager_only_prioritize_repo2(self):
        ...
        self.assertEqual(
            sorted(phases[1]),
            sorted(["repo2/c0/build/linux", "repo2/c1/build/linux"]),
            phases,
        )
    ...
    def test_manager_branch_pinning(self):
        ...
        harness.manager.source_control.addCommit(
            "repo2/c2", ["repo2/c1"], TestYamlFiles.repo2
        )
        ...
        harness.manager.source_control.addCommit(
            "repo2/c3", ["repo2/c2"], TestYamlFiles.repo2
        )
        harness.manager.source_control.setBranch("repo2/master", "repo2/c3")

        # and also repo5
        harness.manager.source_control.addCommit(
            "repo5/c1", ["repo5/c0"], TestYamlFiles.repo5
        )
        ...
        harness.manager.source_control.addCommit(
            "repo2/c4", ["repo2/c3"], TestYamlFiles.repo2
        )
        harness.manager.source_control.setBranch("repo2/master", "repo2/c4")

        def beforePush():
            harness.manager.source_control.addCommit(
                "repo5/c2", ["repo5/c1"], TestYamlFiles.repo5_nopin
            )
        ...
        self.assertEqual(
            harness.manager.source_control.commit_parents[curCommit][0], "repo5/c1"
        )
    ...
    def test_manager_branch_fastforwarding(self):
        ...
        harness.manager.source_control.addCommit(
            "repo2/c5", ["repo2/c4"], TestYamlFiles.repo2
        )
        ...
        with harness.database.view():
            top_commit = harness.getCommit(
                harness.manager.source_control.getBranch("repo5/master")
            )
            self.assertEqual(top_commit.data.repos["child"].reference, "repo2/c5")

            repo, branch, hash, refname = BranchPinning.unpackCommitPinUpdateMessage(
                top_commit.data.commitMessage
            )
        ...
        harness.manager.source_control.addCommit(
            "repo2/c2_alt", ["repo2/c1"], TestYamlFiles.repo2
        )
        harness.manager.source_control.addCommit(
            "repo2/c3_alt", ["repo2/c2_alt"], TestYamlFiles.repo2
        )
        harness.manager.source_control.addCommit(
            "repo2/c4_alt", ["repo2/c3_alt"], TestYamlFiles.repo2
        )
        harness.manager.source_control.addCommit(
            "repo2/c5_alt", ["repo2/c4_alt"], TestYamlFiles.repo2
        )
    ...
    def test_manager_branch_circular_pinning(self):
        ...
        harness.manager.source_control.addCommit(
            "repo6/c0", [], TestYamlFiles.repo6.replace("__branch__", "master1")
        )
        harness.manager.source_control.addCommit(
            "repo6/c1", [], TestYamlFiles.repo6.replace("__branch__", "master2")
        )
        harness.manager.source_control.addCommit(
            "repo6/c2", [], TestYamlFiles.repo6.replace("__branch__", "master3")
        )
        harness.manager.source_control.addCommit(
            "repo6/c3", [], TestYamlFiles.repo6.replace("__branch__", "master4")
        )
        harness.manager.source_control.addCommit(
            "repo6/c4", [], TestYamlFiles.repo6.replace("__branch__", "master0")
        )
        ...
        harness.manager.source_control.addCommit(
            "repo6/c0_alt", [], TestYamlFiles.repo6_nopin
        )
    ...
    def test_manager_pin_resolution_ordering(self):
        ...
        harness.manager.source_control.addCommit(
            "repo6/underlying", [], TestYamlFiles.repo6_nopin
        )
        harness.manager.source_control.setBranch("repo6/root", "repo6/underlying")

        nonauto_pin_contents = (
            TestYamlFiles.repo6_headpin.replace("__branch__", "root")
        ...
        def add(branch, deps):
            if len(deps) == 0:
                ...
                contents = TestYamlFiles.repo6_headpin.replace(
                    "__branch__", deps[0]
                ).replace(
                    "HEAD",
                    harness.manager.source_control.getBranch("repo6/" + deps[0]).split(
                        "/"
                    )[1],
                )
            elif len(deps) == 2:
                contents = (
                    TestYamlFiles.repo6_twopins.replace("__branch__", deps[0])
                    .replace(
                        "HEAD1",
                        harness.manager.source_control.getBranch(
                            "repo6/" + deps[0]
                        ).split("/")[1],
                    )
        ...
        harness.manager.source_control.addCommit(
            "repo6/new_base_commit", [], nonauto_pin_contents
        )
        ...
        harness.manager.source_control.addCommit(
            "repo6/underlying2", [], TestYamlFiles.repo6_nopin
        )
        ...
        with harness.database.transaction():
            branch = harness.database.Branch.lookupOne(
                reponame_and_branchname=("repo6", "b0")
            )
            assert branch
    ...
    def test_manager_update_head_commits(self):
        ...
        harness.manager.source_control.addCommit(
            "repo6/c0", [], TestYamlFiles.repo6_nopin
        )
        harness.manager.source_control.setBranch("repo6/master", "repo6/c0")

        harness.manager.source_control.addCommit(
            "repo6/c1", [], TestYamlFiles.repo6_headpin.replace("__branch__", "master")
        )
    ...
    def test_manager_reachability(self):
        ...
        for commitIx in range(2, 200):
            manager.source_control.addCommit(
                "repo0/c%s" % commitIx,
                ["repo0/c%s" % (commitIx - 1)],
                TestYamlFiles.repo0,
            )
    ...
    def test_manager_env_imports(self):
        ...
        with manager.database.view():
            ...
            commit2deps = manager.database.UnresolvedCommitRepoDependency.lookupAll(
                commit=commit2
            )
            self.assertEqual([x.reponame for x in commit2deps], ["repo1"])

            commit3deps = manager.database.UnresolvedCommitRepoDependency.lookupAll(
                commit=commit3
            )
        ...
        with manager.database.view():
            ...
            self.assertFalse(
                manager.database.UnresolvedCommitRepoDependency.lookupAll(
                    commit=commit1
                )
            )
            self.assertFalse(
    ...
    def test_manager_timeouts(self):
        ...
        harness.manager.startNewTest(
            harness.getUnusedMachineIds()[0], harness.timestamp
        )
        ...
        with harness.database.view():
            self.assertEqual(
                sorted(
                    [
                        namefor(t.test)
                        for t in harness.database.TestRun.lookupAll(isRunning=True)
                    ]
                ),
                sorted(
                    [
                        namefor(harness.lookupTestByFullname(x))
                        for x in ["repo1/c0/build/linux", "repo1/c0/test/windows"]
                    ]
                ),
            )
    ...
    def test_manager_remembers_old_repos(self):
        ...
        with harness.database.view():
            ...
            self.assertEqual(
                repo1._identity, harness.database.Repo.lookupOne(name="repo1")._identity
            )
    ...
    def test_manager_missing_environment_refs(self):
        ...
        for ordering in [(0, 1, 2, 3), (3, 2, 1, 0), (3, 1, 2, 0)]:
            ...
            with harness.database.view():
                self.assertTrue(
                    harness.lookupTestByFullname("repo2/c0/build_without_deps/linux"),
                    ordering,
                )
                self.assertTrue(
                    harness.lookupTestByFullname("repo3/c0/build_without_deps/linux"),
                    ordering,
                )
    ...
    def test_circular_environment_refs(self):
        ...
        harness.manager.source_control.addCommit(
            "repo7/c0", [], TestYamlFiles.repo7_circular
        )
    ...
    def test_circular_test_refs(self):
        ...
        harness.manager.source_control.addCommit(
            "repo8/c0", [], TestYamlFiles.repo8_circular_builds
        )
        ...
        with harness.database.view():
            ...
            self.assertTrue(
                "ircular" in c.data.testDefinitionsError, c.data.testDefinitionsError
            )
    ...
    def test_child_repo_refs(self):
        ...
        harness.add_content()
        harness.manager.source_control.addCommit(
            "repo9/c0", [], TestYamlFiles.repo9_import_child_refs
        )
        ...
        with harness.database.view():
            ...
            self.assertEqual(
                harness.manager.environmentForTest(test0).variables["ENV"], "repo0"
            )
            self.assertEqual(
                harness.manager.environmentForTest(test1).variables["ENV_VAR"], "LINUX"
            )
            self.assertEqual(
                harness.manager.environmentForTest(test2).variables["ENV_VAR_2"],
                "LINUX_2",
            )
    ...
    def test_manager_branch_creation_from_template(self):
        ...
        self.assertTrue(
            "repo6/a_branch-looper" in harness.manager.source_control.listBranches(),
