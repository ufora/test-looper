...
class TestManagerIncludeSemanticsTests(unittest.TestCase):
    def test_basic_includes(self):
        ...
        harness.manager.source_control.addCommit(
            "repo0/base", [], "", {"envdef.yml": repo_include_envdef}
        )
        ...
        self.assertTrue(
            sorted(resolver.environmentsFor("repo0", "c0").keys()) == ["e1", "e2"]
        )
    ...
    def test_include_includes(self):
        ...
        harness.manager.source_control.addCommit(
            "repo0/c0", [], repo, {"envdef.yml": envdef, "envdef2.yml": envdef2}
        )

        resolver = harness.resolver()

        self.assertEqual(
            sorted(resolver.repoReferencesFor("repo0", "c0").keys()), ["r"]
        )
    ...
    def test_env_inheritance_in_included_files(self):
        ...
        harness.manager.source_control.addCommit(
            "repo0/c0", [], "looper_version: 5", {"lowest.yml": lowest}
        )
        ...
        self.assertEqual(
            sorted(resolver.environmentsFor("repo0", "c1").keys()),
            ["derived", "really_derived", "root_env"],
        )
    ...
    def test_includes_can_share_environments(self):
        ...
        self.assertEqual(
            sorted(resolver.testDefinitionsFor("repo0", "c0")), ["t1/e", "t2/e"]
        )
    ...
    def test_bad_include_preserves_pins(self):
        ...
        harness.manager.source_control.addCommit(
            "repo0/c0", [], None, {"envdef.yml": envdef}
        )
        harness.manager.source_control.addCommit(
            "repo0/c1",
            ["repo0/c0"],
            None,
            {"envdef.yml": envdef.replace("not_valid", "linux")},
        )
        ...
        with harness.database.view():
            ...
            branch = harness.database.Branch.lookupOne(
                reponame_and_branchname=("repo0", "tester")
            )
    ...
    def test_environment_overrides(self):
        ...
        self.assertEqual(
            [s.command for s in test.stages],
            ["preCommand", "preCommand2", "actualCommand"],
        )
    ...
    def test_configuration_override(self):
        ...
        self.assertEqual(
            resolver.testDefinitionsFor("repo0", "c0")["t1"].configuration,
            "override_at_root",
        )
        self.assertEqual(
            resolver.testDefinitionsFor("repo0", "c0")["t2"].configuration,
            "override_at_mixin",
        )
        self.assertEqual(
            resolver.testDefinitionsFor("repo0", "c0")["t3"].configuration,
            "override_at_test_level",
        )
    ...
    def test_prioritization_filters(self):
        ...
        self.assertEqual(
            set([t.name for t in tests]), set(["t1/e1", "t1/e2", "t2/e2", "t2/e1"])
        )

        harness.manager.source_control.addCommit(
            "repo0/c1", ["repo0/c0"], envdef, {"a/file": "stuff"}
        )
        fullTestSets, testSets, triggeredTestSets, triggeredTriggerNames = resolver.testSetsFor(
            "repo0", "c1"
        )
        ...
        harness.manager.source_control.addCommit(
            "repo0/c2", ["repo0/c0"], envdef, {"b/file": "stuff"}
        )
        fullTestSets, testSets, triggeredTestSets, triggeredTriggerNames = resolver.testSetsFor(
            "repo0", "c2"
        )
    ...
    def test_environment_mixins(self):
        ...
        self.assertEqual(
            [s.command for s in test.stages],
            ["preCommand", "preCommand2", "actualCommand - v=e2"],
        )
