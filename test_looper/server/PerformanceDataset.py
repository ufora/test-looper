...
class PerformanceObservationList(object):
    ...
    def __add__(self, other):
        ...
        if self.seriesName == other.seriesName:
            ...
            assert self.count is not None, "seriesName = %s, commit = %s, mean = %s, std = %s, count %s" % (self.seriesName, self.commit, self.mean, self.std, self.count)
            ...
            newSeriesName = os.path.commonprefix([self.seriesName, other.seriesName])
            ix = newSeriesName.rfind('.')
            if ix > 0:
                newSeriesName = newSeriesName[:ix]
...
class PerformanceDataset(object):
    ...
    def getAllGroupSeriesData(self):
        #first filter to the set we care about
        dataBySeries = {k: v for k, v in self.dataBySeries.iteritems()}
    ...
    def groupSeriesData(self, prefix):
        ...
        for seriesName in dataBySeries:
            ...
            if ix > 0:
                ...
                isTerminal[seriesName[:ix]] = False
            else:
                groups.add(seriesName)
                isTerminal[seriesName] = True
    ...
    def getObservationData(self):
        observations = []
        dataBySeries = self.getAllGroupSeriesData()
        for (name, series) in dataBySeries.iteritems():
            observation = {}
            observation["name"] = name
            commits = []
            for (commit, data) in series.observations.iteritems():
                commitStats = {}
                commitStats["commit"] = commit
                commitStats["mean"] = data.mean
                commitStats["std"] = data.std
                commitStats["count"] = data.count
                commits.append(commitStats)
            observation["commits"] = commits
