...
def createArgumentParser():
    ...
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Set logging level to verbose"
    )

    parser.add_argument(
        "--repocheck", action="store_true", help="Print the set of known repos and exit"
    )
...
def exportToFile(testManager, exportDir):
    exportPath = os.path.join(
        exportDir, time.strftime("%Y%m%d-%H%M%S-UTC.yml", time.gmtime())
    )
...
def main():
    ...
    config = algebraic_to_json.Encoder(mergeListsIntoDicts=False).from_json(
    ...
    if config.server.database.matches.InMemory:
        ...
        jsonStore = RedisJsonStore(
            port=config.server.database.port or None, db=config.server.database.db
        )

    eventLog = TestLooperHttpServerEventLog(jsonStore)

    src_ctrl = SourceControlFromConfig.getFromConfig(
        config.server.path_to_local_repos, config.source_control
    )
    ...
    artifact_storage.tempfileOverrideDir = os.path.join(
        config.server.path_to_local_repos, "tarballs"
    )
    ...
    machine_management = MachineManagement.fromConfig(
        config, src_ctrl, artifact_storage
    )

    testManager = TestManager.TestManager(
        config.server_ports, src_ctrl, machine_management, jsonStore
    )
    ...
    server = TestLooperServer.TestLooperServer(
        config.server_ports, testManager, httpServer, machine_management
    )
