...
class IndividualTestContext(Context.Context):
    ...
    def urlBase(self):
        return (
            self.contextFor(self.context).urlBase()
            + "/-/individualTest/"
            ...
            for test in self.context.commit.data.tests.values():
                if self.context.shouldIncludeTest(test):
                    for run in self.database.TestRun.lookupAll(test=test):
                        if (
                            run.testNames
                            and self.individualTestName in run.testNames.test_names
                        ):
                            res.append(run)
            return res
        ...
        if self.options.get("context", "") == "dropdown-menu":
            ...
            for testRun in self.relevantTestRuns():
                for (
                    path,
                    sz,
                ) in self.renderer.artifactStorage.testResultKeysAndSizesForIndividualTest(
                    ...
                    if sz:
                        items.append(
                            '<a class="dropdown-item" href="{link}" title="{title}">{contents}</a>'.format(
                                link=self.renderer.testResultDownloadUrl(
                        ...
                        items.append(
                            '<span class="dropdown-item disabled text-muted">{contents}</span>'.format(
                                contents=contents
                            )
                        )
            return "".join(items)
            ...
            for testRun in [
                t for t in self.relevantTestRuns() if not t.canceled and t.endTimestamp
            ]:
                timesSeen = -1
                        ...
                        for path, sz in sorted(pathsAndSizes):
                            grid.append(
                                [
                                    ...
                                    "OK"
                                    if testRun.testStepSucceeded[ix] is True
                                    else "FAIL",
                                    HtmlGeneration.link(
                                        os.path.basename(path),
