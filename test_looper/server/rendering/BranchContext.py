...
class BranchContext(Context.Context):
    def __init__(
        self, renderer, branch, configurationFilter, projectFilter, parentLevel, options
    ):
    ...
    def appropriateLinkName(self, isInMenu):
        if self.parentLevel == 0 and isInMenu:
            return (
                ...
                or '<span class="text-muted">all configs</span>'
            )
    ...
    def renderNavbarLink(self, isInMenu=False):
        return octicon(self.appropriateIcon(isInMenu)) + self.renderLink(
            includeRepo=False, isInMenu=isInMenu
        )
    ...
    def primaryObject(self):
        return ComboContexts.BranchAndFilter(
            self.branch, self.configurationFilter, self.projectFilter, self.parentLevel
        )
    ...
    def pruneCommitList(self, commits, parents, children):
        ...
        def shouldIncludeTest(test):
            ...
            if (
                self.configurationFilter
                and test.testDefinitionSummary.configuration != self.configurationFilter
            ):
                return False
            return True
        ...
        def wantsCommit(c):
            ...
            for testSet, tests in c.data.testSetsTopLevel.items():
                if (
                    testSet in c.data.triggeredTestSets
                    or testSet in c.userEnabledTestSets
                ):
                    for t in tests:
        ...
        for c in commits:
            if (
                not wantsCommit(c)
                and len(parents.get(c.hash, [])) <= 1
                and len(children.get(c.hash, [])) <= 1
            ):
    ...
    def testDisplayForCommits(self, commits):
        ...
        for commit_ix, c in enumerate(commits):
            ...
            if len(parentsWeHave) == 0:
                ...
                commit_string += (
                    "%s.commit({sha1: '%s', message: '%s', detailId: 'commit_%s'});\n"
                    % (
                        branches[c.hash],
                        c.hash,
                ...
                commit_string += (
                    "%s.merge(%s, {sha1: '%s', message: '%s', detailId: 'commit_%s'}).delete();"
                    % (
                        other_branch,
                        our_branch,
                        c.hash,
        ...
        for c in reversed(commits):
            ...
            if c.hash in discardRoots:
                grid.append(
                    [
                        {
                            "content": '<span class="text-muted">%s commits filtered out</span>'
                            % discardRoots[c.hash],
                            "colspan": len(grid[0]),
                        }
                    ]
    ...
    def getGridRenderer(self, commits):
        ...
        if (
            self.options.get("expanded_columns")
            or len(projects) == 1
            ...
            if not projectFilter:
                return TestGridRenderer.TestGridRenderer(
                    ...
                    lambda group: self.contextFor(
                        ComboContexts.BranchAndFilter(
                            self.branch, configFilter, group, parentLevel=1
                        )
                    ).renderNavbarLink(isInMenu=True),
            ...
            if not configFilter:
                return TestGridRenderer.TestGridRenderer(
                    ...
                    lambda group: self.contextFor(
                        ComboContexts.BranchAndFilter(
                            self.branch, group, projectFilter, parentLevel=0
                        )
                    ).renderNavbarLink(isInMenu=True),
                    lambda group, row: self.contextFor(
            ...
            return TestGridRenderer.TestGridRenderer(
                ...
                lambda test: test.testDefinitionSummary.project
                + " / "
                + test.testDefinitionSummary.configuration,
                cacheName=cacheKey,
                database=self.testManager.database,
            )
        else:
            ...
            return TestGridRenderer.TestGridRenderer(
                ...
                .renderLink()
                .withTextReplaced(
                    "%s projects over %s configurations"
                    % (len(projects), len(configurations))
                ),
                lambda group, row: self.contextFor(
                    ComboContexts.CommitAndFilter(row, configFilter, projectFilter)
                ).urlString(),
        ...
        if self.configurationFilter or self.projectFilter:
            return self.contextFor(
                ComboContexts.CommitAndFilter(
                    commit, self.configurationFilter, self.projectFilter
                )
            )
    ...
    def getBranchCommitRow(self, commit, renderer):
        ...
        if all_tests:
            row[-1] += (
                "&nbsp;" + self.contextFor(commit).dropdownForTestPrioritization()
            )
    ...
    def topNCommitTestSummaryRow(self, N):
        ...
        for commit in self.renderer.testManager.topNPrioritizedCommitsForBranch(
            self.branch, N
        ):
            testRow.append(
    ...
    def consumePath(self, path):
        if path and path[0] == "configurations":
            ...
            return (
                self.contextFor(
                    ComboContexts.BranchAndFilter(
                        self.branch, configurationName, self.projectFilter
                    )
                ),
                remainder,
            )
        ...
        if path and path[0] == "projects":
            ...
            return (
                self.contextFor(
                    ComboContexts.BranchAndFilter(
                        self.branch, self.configurationFilter, projectName
                    )
                ),
                remainder,
            )
    ...
    def childContexts(self, currentChild):
        if isinstance(
            currentChild.primaryObject(),
            (self.database.Commit, ComboContexts.CommitAndFilter),
        ):
            ...
            return [
                self.contextFor(
                    ComboContexts.CommitAndFilter(
                        x, self.configurationFilter, self.projectFilter, 2
                    )
                )
                for x in list(
                    reversed(
                        self.testManager.getNCommits(
                            commit, 10, "above", commitsInBetween
                        )
                    )
                )
                + [commit]
            ...
            if currentChild.parentLevel == 0:
                ...
                return [
                    self.contextFor(
                        ComboContexts.BranchAndFilter(
                            branch=self.branch,
                            configurationName=self.configurationFilter,
                            projectName=g,
                            parentLevel=1,
                        )
                    )
        ...
        return sorted(
            set(
                [
                    ...
                    for commit in testManager.commitsToDisplayForBranch(
                        branch, maxCommitCount
                    )
                    for t in testManager.allTestsForCommit(commit)
                    if t.testDefinitionSummary.project == projectFilter
                    or not projectFilter
                ]
            )
        )
        ...
        return sorted(
            set(
                [
                    ...
                    for t in testManager.allTestsForCommit(commit)
                    if t.testDefinitionSummary.configuration == configFilter
                    or not configFilter
                ]
            )
        )
    ...
    def renderPostViewSelector(self):
        ...
        if self.configurationFilter or self.projectFilter:
            res.append(
                HtmlGeneration.Link(
                    self.withOptions(
                        show_all_commits="true" if not is_show_all_commits else None
                    ).urlString(),
                    "Show All Commits",
                    is_button=True,
                    button_style="btn-%s btn-xs"
                    % ("outline-primary" if not is_show_all_commits else "primary"),
        ...
        res.append(
            HtmlGeneration.Link(
                self.withOptions(
                    expanded_columns="true" if not is_detail else None
                ).urlString(),
                "Detail View",
                is_button=True,
                button_style="btn-%s btn-xs"
                % ("outline-primary" if not is_detail else "primary"),
    ...
    def renderMenuItemText(self, isHeader):
        return (
            octicon(self.appropriateIcon(True)) if isHeader else ""
        ) + self.appropriateLinkName(True)
