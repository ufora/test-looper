...
windows_ami_creator = open(
    os.path.join(own_dir, "bootstraps", "windows_ami_creator.ps1"), "r"
).read()
windows_bootstrap_script = open(
    os.path.join(own_dir, "bootstraps", "windows_bootstrap.ps1"), "r"
).read()
linux_bootstrap_script = open(
    os.path.join(own_dir, "bootstraps", "linux_bootstrap.sh"), "r"
).read()
...
class API:
    def __init__(self, config):
        ...
        self.ec2 = boto3.resource(
            "ec2", region_name=self.config.machine_management.region
        )
        self.ec2_client = boto3.client(
            "ec2", region_name=self.config.machine_management.region
        )
        self.s3 = boto3.resource(
            "s3", region_name=self.config.machine_management.region
        )
        ...
        filters = [
            {
                "Name": "tag:Name",
                "Values": [
                    self.config.machine_management.worker_name
                    + ("_ami_creator" if producingAmis else "")
                ],
            }
        ]
        res = []
    ...
    def isInstanceWeOwn(self, instance):
        ...
        if not [
            ...
            if t["Key"] == "Name"
            and t["Value"] == self.config.machine_management.worker_name
        ]:
            return False
        ...
        if not [
            t
            for t in instance.security_groups
            if t["GroupId"] == self.config.machine_management.security_group
        ]:
            return False
    ...
    def generateAmiConfigLogUrl(self, baseAmi, setupScriptHash, logType):
        ...
        log_objects = bucket.objects.filter(
            Prefix=self.bootstrap_key_root
            + baseAmi
            + "_"
            + setupScriptHash
            + "_"
        ...
        for log in log_objects:
            Params = {
                "Bucket": self.config.machine_management.bootstrap_bucket,
                "Key": log.key,
            }
    ...
    def listWindowsImages(self, availableOnly):
        ...
        for i in images:
            ...
            if (
                tags.get("testlooper_worker_name", "")
                == self.config.machine_management.worker_name
                and "BaseAmi" in tags
                and "SetupScriptHash" in tags
            ):
    ...
    def gatherAmis(self):
        ...
        for ami, hash in set(list(instances.keys()) + list(images.keys())):
            try:
                self.checkAmiStateTransition(
                    ami,
                    hash,
                    instances.get((ami, hash), None),
                    images.get((ami, hash), None),
                    amiStates.get((ami, hash), None),
                )
            except:
                logging.error(
                    "Failed to update AMI state transition for %s/%s:\n%s",
                    ami,
                    hash,
                    traceback.format_exc(),
                )

    def checkAmiStateTransition(
        self, baseAmi, scriptHash, creatorInstance, actualImage, bootstrapLogState
    ):
        if actualImage is not None:
            ...
            if bootstrapLogState == "Failed":
                logging.info(
                    "Instance %s produced a failure bootstrap log for %s/%s, so shutting it down",
                    creatorInstance,
                    baseAmi,
                    scriptHash,
                )
            ...
            if creatorInstance.state["Name"] == "stopped":
                logging.info(
                    "Ami %s/%s has a stopped instance but no image. Creating one.",
                    baseAmi,
                    scriptHash,
                )
                for v in creatorInstance.volumes.all():
                    ...
                    if "/dev/xvdb" in devices:
                        ...
                        v.create_tags(
                            Tags=[
                                {
                                    "Key": "testlooper_worker_name",
                                    "Value": self.config.machine_management.worker_name,
                                },
                                {
                                    "Key": "testlooper_volume_type",
                                    "Value": "image_bootstrap_discardable_storage",
                                },
                            ]
                        )
                ...
                if not image:
                    ...
                    logging.info(
                        "Image named %s already exists as %s", imageName, image
                    )
                ...
                if upFor > 3600 * 2:
                    logging.info(
                        "Instance %s has been up for %s seconds, which exceeds the cutoff",
                        creatorInstance,
                        upFor,
                    )

                    bucket = self.s3.Bucket(
                        self.config.machine_management.bootstrap_bucket
                    )
                    bucket.put_object(
                        Key=self.bootstrap_key_root
                        + baseAmi
                        + "_"
                        + setupScriptHash
                        + "_BootstrapLog.fail",
    ...
    def instanceLaunchTimestamp(self, instance):
        launch_naive = (
            instance.launch_time.replace(tzinfo=None) - instance.launch_time.utcoffset()
        )
    ...
    def lookupActualAmiForScriptHash(self, baseAmi, setupScriptHash):
        res = self.listWindowsImages(availableOnly=True).get(
            (baseAmi, setupScriptHash), None
        )
        assert res is not None, "Image %s/%s is not available" % (
            baseAmi,
            setupScriptHash,
        )
        return res.id
    ...
    def bootAmiCreator(self, platform, instanceType, baseAmi, setupScript):
        ...
        if (baseAmi, setupScriptHash) in self.imagesBeingProducedByWorkers():
            logging.warn(
                "We tried to boot an image creator for %s/%s, but one is already up",
                (baseAmi, setupScriptHash),
            )
            return

        if (baseAmi, setupScriptHash) in self.listWindowsImages(availableOnly=True):
            logging.warn(
                "We tried to boot an image creator for %s/%s, but the image already exists!",
                (baseAmi, setupScriptHash),
            )
            return

        logging.info(
            "Booting an AMI creator for ami %s and hash %s", baseAmi, setupScriptHash
        )
        ...
        looper_server_and_port = "%s:%s" % (
            self.config.server_ports.server_address,
            self.config.server_ports.server_https_port,
        )

        bootstrap_path = (
            self.bootstrap_key_root + baseAmi + "_" + sha_hash(setupScript).hexdigest
        )

        boot_script = (
            windows_ami_creator.replace(
                "__windows_box_password__",
                self.config.machine_management.windows_password,
            )
            .replace(
                "__test_key__",
                open(self.config.machine_management.path_to_keys, "r").read(),
            )
            .replace(
                "__test_key_pub__",
                open(self.config.machine_management.path_to_keys + ".pub", "r").read(),
            )
            .replace(
                "__bootstrap_bucket__", self.config.machine_management.bootstrap_bucket
            )
            ...
            .replace(
                "__hosts__",
                "\n\n".join(
                    'echo "%s %s" |  Out-File -Append c:/Windows/System32/Drivers/etc/hosts -Encoding ASCII'
                    % (ip, hostname)
                    for hostname, ip in self.config.machine_management.host_ips.items()
                ),
            )
        )
        ...
        reboot_script = windows_bootstrap_script.replace(
            "__test_config__",
            json.dumps(
                {
        ...
        self.bootWorker(
            ...
            nameValueOverride=self.config.machine_management.worker_name
            + "_ami_creator",
    ...
    def bootWorker(
        ...
        if platform == "linux":
            boot_script = (
                linux_bootstrap_script.replace(
                    "__test_key__",
                    open(self.config.machine_management.path_to_keys, "r").read(),
                )
                .replace(
                    "__test_key_pub__",
                    open(
                        self.config.machine_management.path_to_keys + ".pub", "r"
                    ).read(),
                )
                .replace(
                    "__test_config__",
                ...
                .replace(
                    "__test_looper_https_server__",
                    self.config.server_ports.server_address,
                )
                .replace(
                    "__test_looper_https_port__",
                    str(self.config.server_ports.server_https_port),
                )
                .replace(
                    "__hosts__",
                    "\n\n".join(
