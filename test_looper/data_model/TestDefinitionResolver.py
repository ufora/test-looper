...
class MissingDependencyException(TestResolutionException):
    ...
    def __str__(self):
        ...
        return "MissingDependencyException(repo=%s, commit=%s)" % (
            self.reponame,
            self.commitHash,
        )
...
class TestDefinitionResolver:
    ...
    def unprocessedTestsEnvsAndReposFor_(self, repoName, commitHash):
        ...
        if textAndExtension is None or textAndExtension[1] is None:
            ...
            self.rawDefinitionsCache[
                repoName, commitHash
            ] = TestDefinitionScript.extract_tests_from_str(
                repoName, commitHash, textAndExtension[1], textAndExtension[0]
            )
    ...
    def resolveRepoDefinitions_(self, curRepoName, curCommitHash, raw_repos):
        ...
        for r in raw_repos:
            if raw_repos[r].matches.Import:
                ...
                if raw_repos[r].reference == "HEAD" and curRepoName != "__root":
                    repos[r] = raw_repos[r]._withReplacement(
                        reference=curRepoName + "/" + curCommitHash
                    )
                else:
                    repos[r] = raw_repos[r]

        if (
            any([r.commitHash() == "HEAD" for r in repos.values()])
            and curRepoName != "__root"
        ):
            return {
                r: v for r, v in repos.items() if v.matches.Pin or v.matches.Reference
            }
        ...
        def resolveRepoRef(refName, ref, pathSoFar):
            ...
            if not ref.matches.Import:
                return self.resolveRefWithinRepo(
                    curRepoName, curCommitHash, refName, ref
                )
            ...
            if importSeq[0] not in repos:
                raise TestResolutionException(
                    "Can't resolve reference to repo def %s" % (importSeq[0])
                )

            subref_parent_repo = curRepoName
            subref = resolveRepoRef(
                importSeq[0], repos[importSeq[0]], pathSoFar + (ref,)
            )

            for s in importSeq[1:]:
                subref_parent_repo = subref.reponame()

                repos_for_subref = self.repoReferencesFor(
                    subref.reponame(), subref.commitHash()
                )

                if s not in repos_for_subref:
                    raise TestResolutionException(
                        "Can't resolve reference %s because %s/%s doesn't have %s"
            ...
            subref = TestDefinition.RepoReference.ImportedReference(
                ...
                orig_reference=""
                if subref.matches.Reference
                else subref.orig_reference
                if subref.matches.ImportedReference
    ...
    def resolveIncludeString_(self, repos, repoName, commitHash, path):
        ...
        if not (items[0] == "" or items[0] in repos or items[0] in (".", "..")):
            raise TestResolutionException(
                "Invalid include %s: should start with a repo, a '/' (for root of current repo), '.', or '..'"
                % i.path
            )
        ...
        if items[0] in (".", ".."):
            return (
                ...
                commitHash,
                resolvePath(
                    self.rawDefinitionsPath[repoName, commitHash] + "/../" + path
                ),
            )
        ...
        if items[0] in repos:
            ...
            return (
                repoRef.reponame(),
                repoRef.commitHash(),
                pathJoin(repoRef.path, "/".join(items[1:])),
            )
    ...
    def _computeAllDiffs(self, repoName, commitHash):
        ...
        return [
            "HEAD/" + x for x in repo.filesChangedBetweenCommits(commitHash, parents[0])
        ]
    ...
    def testSetsFor(self, repoName, commitHash):
        ...
        repos, _, _, _, test_sets_with_globs, triggers = self.postIncludeDefinitions_(
            repoName, commitHash
        )
        ...
        if not triggers:
            triggers = [
                TestDefinitionScript.PrioritizationTrigger.Trigger(
                    name="", paths=["*"], test_sets=["all"]
                )
            ]
        ...
        if allDiffs is EVERYTHING:
            logging.info(
                "Commit %s is considered to have EVERYTHING as its diffs.", commitHash
            )
        else:
            logging.info(
                "Commit %s has diffs:\n%s",
        ...
        if allDiffs is EVERYTHING:
            ...
            for t in triggers:
                logging.info(
                    "On commit %s, trigger %s applies: %s with diffs %s",
                    commitHash,
                    t,
                    self._triggerApplies(allDiffs, t),
                    allDiffs,
                )
        ...
        self.triggeredTestSetsCache[repoName, commitHash] = (
            fullTestSets,
            testSets,
            triggeredTestSets,
            triggeredTriggerNames,
        )
    ...
    def postIncludeDefinitions_(self, repoName, commitHash):
        ...
        tests, envs, repos, includes, test_sets, triggers = self.unprocessedTestsEnvsAndReposFor_(
            repoName, commitHash
        )
        ...
        while includes:
            ...
            includeRepo, includeHash, includePath = self.resolveIncludeString_(
                repos, includeSourceRepo, includeSourceHash, i.path
            )
            ...
            if include_key not in everIncluded:
                ...
                if attempts > MAX_INCLUDE_ATTEMPTS:
                    raise TestResolutionException(
                        "Exceeded the maximum number of file includes: %s"
                        % MAX_INCLUDE_ATTEMPTS
                    )

                everIncluded.add(include_key)

                contents = self.getRepoContentsAtPath(
                    includeRepo, includeHash, includePath
                )

                if contents is None:
                    raise TestResolutionException(
                        "Can't find path %s in %s/%s"
                        % (includePath, includeRepo, includeHash)
                    )
        ...
        self.postIncludeDefinitionsCache[repoName, commitHash] = (
            tests,
            envs,
            repos,
            includes,
    ...
    def assertEnvironmentsNoncircular_(self, environments, repoName, commitHash):
        ...
        cycle = GraphUtil.graphFindCycleMultipleRoots(
            [
                TestDefinition.EnvironmentReference(
                    repo=repoName, commitHash=commitHash, name=e
                )
        ...
        if cycle:
            raise TestResolutionException(
                "Circular environment dependency found: %s" % (" -> ".join(cycle))
            )
    ...
    def resolveEnvironmentPreMerge_(self, environment, resolved_repos):
        ...
        def resolveEnvironmentReference(env):
            ...
            if env.repo_name not in resolved_repos:
                ...
                raise TestResolutionException(
                    "Environment depends on unknown reponame: %s" % env.repo_name
                )

            ref = resolved_repos[env.repo_name]

            return TestDefinition.EnvironmentReference(
                repo=ref.reponame(), commitHash=ref.commitHash(), name=env.name
            )
            ...
            if image.matches.Dockerfile:
                ...
                if contents is None:
                    raise TestResolutionException(
                        ...
                        % (image.dockerfile, image.repo, image.commitHash)
                    )

                return TestDefinition.Image.DockerfileInline(contents)
            return image

        environment = environment._withReplacement(
            dependencies={
                depname: resolveTestDep(dep)
                for depname, dep in environment.dependencies.items()
            }
        )

        if environment.matches.Environment:
            environment = environment._withReplacement(
                image=resolveImage(environment.image)
            )
        else:
            environment = environment._withReplacement(
                imports=[resolveEnvironmentReference(i) for i in environment.imports]
            )
    ...
    def actualEnvironmentNameForTest_(self, testDef):
        if not testDef.environment_mixins:
            ...
            return "+".join(
                [testDef.environment_name] + list(testDef.environment_mixins)
            )
    ...
    def environmentsFor(self, repoName, commitHash):
        ...
        for testDef in tests.values():
            if testDef.environment_mixins:
                ...
                fakeEnvironment = TestDefinition.TestEnvironment.Import(
                    ...
                    inheritance="",
                    imports=[
                        TestDefinition.EnvironmentReference.Reference(
                            repo=repoName, commitHash=commitHash, name=ref
                        )
                        for ref in [testDef.environment_name]
                        + list(testDef.environment_mixins)
                    ],
        ...
        environments = {
            e: self.resolveEnvironmentPreMerge_(environments[e], resolved_repos)
            for e in environments
        }
        ...
        def resolveEnvironment(environment):
            ...
            def import_dep(dep):
                ...
                if not underlying_env:
                    raise TestResolutionException(
                        "Can't find environment %s for %s/%s. Available: %s"
                        % (dep.name, dep.repo, dep.commitHash, ",".join(env_set))
                    )
    ...
    def assertTestsNoncircular_(self, tests):
            ...
            try:
                return (
                    [
                        ...
                        for dep in tests[t].dependencies.values()
        ...
        cycle = GraphUtil.graphFindCycleMultipleRoots(tests, children)

        if cycle:
            raise TestResolutionException(
                "Circular test dependency found: %s" % (" -> ".join(cycle))
            )

    def resolveTestNameToTestAndArtifact(
        self, testName, testSet, ignoreArtifactResolution=False
    ):
            ...
            if "/".join(parts[:i]) in testSet:
                ...
                if not ignoreArtifactResolution:
                    ...
                    if artifact not in validArtifacts:
                        raise UserWarning(
                            "Can't resolve artifact '%s' in test %s. Valid are %s."
        ...
        raise UserWarning(
            "Can't resolve %s to a valid name amongst:\n%s"
    ...
    def mostRecentHashForSubpath(self, repo, commitHash, path):
        ...
        if not res:
            raise UserWarning(
                "Can't find a recent hash in %s/%s path %s" % (repo, commitHash, path)
            )
    ...
    def testDefinitionsFor(self, repoName, commitHash):
        ...
        def resolveTestEnvironmentAndApplyVars(testDef):
            ...
            if name not in resolved_envs:
                raise TestResolutionException(
                    "Can't find environment %s (referenced by %s) in\n%s"
                    % (
        ...
        def resolveTestDep(testDep):
            ...
            if testDep.matches.InternalBuild:
                name, artifact = self.resolveTestNameToTestAndArtifact(
                    testDep.name, tests
                )

                return TestDefinition.TestDependency.Build(
                    buildHash=resolveTest(name).hash, name=name, artifact=artifact
                )

            if testDep.matches.ExternalBuild:
                assert not (
                    testDep.repo == repoName and testDep.commitHash == commitHash
                ), testDep

                externalTests = self.testDefinitionsFor(
                    testDep.repo, testDep.commitHash
                )

                name, artifact = self.resolveTestNameToTestAndArtifact(
                    testDep.name, externalTests
                )

                return TestDefinition.TestDependency.Build(
                    buildHash=externalTests[name].hash, name=name, artifact=artifact
                )

            if (
                testDep.matches.UnresolvedExternalBuild
                or testDep.matches.UnresolvedSource
            ):
                if testDep.repo_name not in resolved_repos:
                    raise TestResolutionException(
                        "Test depends on unknown reponame: %s" % testDep.repo_name
                    )
                ...
                try:
                    ...
                    for stage in testDef.stages:
                        if stage.matches.TestStage and not testDef.matches.Test:
                            raise TestResolutionException(
                                "Can't have a 'TestStage' in a build. (%s)" % testName
                            )

                    resolved_tests[testName] = testDef._withReplacement(
                        dependencies={
                            k: resolveTestDep(v)
                            for k, v in testDef.dependencies.items()
                        },
            ...
            for a in stage.artifacts:
                if a.name in validArtifacts:
                    if not a.name:
                        raise UserWarning(
                            (
                                "Test %s defined the unnamed artifact twice. "
                    ...
                    raise UserWarning(
                        "Test %s defined artifact %s twice"
                        % (testDef.name, repr(a.name))
                    )
                validArtifacts.add(a.name)
        if "" in validArtifacts and len(validArtifacts) > 1:
            raise UserWarning(
                "Test %s can only define the unnamed artifact if it defines no others"
                % (testDef.name)
            )
