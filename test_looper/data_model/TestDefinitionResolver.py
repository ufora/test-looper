...
class TestDefinitionResolver:
    ...
    def testEnvironmentAndRepoDefinitionsFor_(self, repoName, commitHash):
        ...
        def resolveTest(testDef):
            ...
            args = testDef._fields
            which = testDef._which

            args["dependencies"] = {k:resolveTestDep(v) for k,v in args["dependencies"].iteritems()}
            args["environment"] = resolveEnvironment(args["environment"])

            return getattr(TestDefinition.TestDefinition,which)(**args)
        ...
        for thing in [resolved_envs, resolved_tests, resolved_repos]:
            if "Unreso" in str(thing):
                print thing
