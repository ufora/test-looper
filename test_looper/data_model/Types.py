...
def setup_types(database):
    ...
    database.FullyResolvedTestEnvironment = algebraic.Alternative(
        "FullyResolvedTestEnvironment"
    )
    database.FullyResolvedTestEnvironment.Unresolved = {}
    database.FullyResolvedTestEnvironment.Error = {"Error": str}
    database.FullyResolvedTestEnvironment.Resolved = {
        "Environment": TestDefinition.TestEnvironment
    }
    ...
    database.CommitTestDependency.define(commit=database.Commit, test=database.Test)

    database.CommitRelationship.define(child=database.Commit, parent=database.Commit)
    ...
    database.TestResultSummary.define(
        ...
        testTotalRuns=algebraic.List(
            int
        ),  # for each test, by index, the total number of times it ran
        testHasLogs=algebraic.List(
            int
        ),  # for each test, by index, how many runs have logs
        testTotalFailures=algebraic.List(
            int
        ),  # for each test, by index, the total number of failures
    ...
    database.UnresolvedTestDependency.define(
        test=database.Test, dependsOnHash=str, artifact=str
    )

    database.UnresolvedCommitSourceDependency.define(
        commit=database.Commit, repo=database.Repo, commitHash=str
    )

    database.UnresolvedCommitRepoDependency.define(commit=database.Commit, reponame=str)

    database.TestDependency.define(
        test=database.Test, dependsOn=database.Test, artifact=str
    )
    ...
    database.TestRun.define(
        ...
        testStepNameIndex=algebraic.List(
            int
        ),  # for each step in the run, which test was it
        testStepTimeStarted=algebraic.List(
            algebraic.Nullable(float)
        ),  # for each step in the run, how long did it take.
        testStepTimeElapsed=algebraic.List(
            algebraic.Nullable(float)
        ),  # for each step in the run, how long did it take.
    ...
    database.IndividualTestNameSet.define(shaHash=str, test_names=algebraic.List(str))
    ...
    database.LogMessage.define(msg=str, timestamp=float, prior=database.LogMessage)
    ...
    database.addIndex(
        database.DataTask, "status", lambda d: d.status if d.isHead else None
    )
    database.addIndex(
        database.DataTask,
        "pending_boot_machine_check",
        lambda d: True
        if d.status.matches.Pending and d.task.matches.BootMachineCheck
        else None,
    )
    database.addIndex(
        database.DataTask,
        "update_commit_priority",
        lambda d: d.task.commit if d.task.matches.UpdateCommitPriority else None,
    )
    database.addIndex(
        database.DataTask,
        "update_test_priority",
        lambda d: d.task.test if d.task.matches.UpdateTestPriority else None,
    )
    ...
    database.addIndex(
        database.Machine, "isAlive", lambda m: True if m.isAlive else None
    )
    database.addIndex(
        database.Machine,
        "hardware_and_os",
        lambda m: (m.hardware, m.os) if m.isAlive else None,
    )

    database.addIndex(
        database.MachineCategory, "hardware_and_os", lambda m: (m.hardware, m.os)
    )
    database.addIndex(
        database.MachineCategory,
        "want_more",
        lambda m: True if (m.desired > m.booted) else None,
    )
    database.addIndex(
        database.MachineCategory,
        "want_less",
    ...
    database.addIndex(
        database.UnresolvedTestDependency,
        "test_and_depends",
        lambda o: (o.test, o.dependsOnHash, o.artifact),
    )
    ...
    database.addIndex(
        database.UnresolvedCommitRepoDependency,
        "commit_and_reponame",
        lambda o: (o.commit, o.reponame),
    )
    database.addIndex(database.UnresolvedCommitSourceDependency, "commit")
    database.addIndex(
        database.UnresolvedCommitSourceDependency,
        "repo_and_hash",
        lambda o: (o.repo, o.commitHash),
    )
    database.addIndex(
        database.UnresolvedCommitSourceDependency,
        "commit_and_repo_and_hash",
        lambda o: (o.commit, o.repo, o.commitHash),
    )

    database.addIndex(database.TestDependency, "test")
    database.addIndex(database.TestDependency, "dependsOn")
    database.addIndex(
        database.TestDependency,
        "test_and_depends",
        lambda o: (o.test, o.dependsOn, o.artifact),
    )
    ...
    database.addIndex(
        database.Branch,
        "reponame_and_branchname",
        lambda o: (o.repo.name, o.branchname),
    )
    ...
    database.addIndex(
        database.BranchPin,
        "pinned_to",
        lambda o: (o.pinned_to_repo, o.pinned_to_branch),
    )
    ...
    database.addIndex(
        database.Deployment,
        "isAliveAndPending",
        lambda d: d.isAlive and not d.machine or None,
    )
    database.addIndex(
        database.Deployment,
        "runningOnMachine",
        lambda d: d.machine if d.isAlive else None,
    )
    database.addIndex(database.Test, "hash")
    database.addIndex(
        database.Test,
        "machineCategoryAndPrioritized",
        lambda o: o.machineCategory
        if (
    ...
    database.addIndex(
        database.TestRun,
        "isRunning",
        lambda t: True if not t.canceled and t.endTimestamp <= 0.0 else None,
    )
    database.addIndex(
        database.TestRun,
        "runningOnMachine",
        lambda t: t.machine if not t.canceled and t.endTimestamp <= 0.0 else None,
    )
